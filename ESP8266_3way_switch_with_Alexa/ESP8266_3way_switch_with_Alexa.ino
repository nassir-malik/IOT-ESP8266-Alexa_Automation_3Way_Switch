...
void setup() {
  ...
  // Setup electrical switch pin for input
  pinMode(switchPin, INPUT);
  
  switchOldState = digitalRead(switchPin);
...
void loop() {
  ...
  //check the state of electrical switch and turn on or off the relay
      currentState = digitalRead(switchPin);
      currentAlexaState = digitalRead(relayPin);
      if (switchOldState != currentState){
          if(currentAlexaState==HIGH ){
            digitalWrite(relayPin, LOW);
          }else{
            digitalWrite(relayPin, HIGH);
          }
          Serial.println("Electrical switch state");
          switchOldState = digitalRead(switchPin);
